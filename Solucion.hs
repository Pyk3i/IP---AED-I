...
cifrado :: String -> [String] -> Int -> String
cifrado frase [] _ = frase
cifrado frase lista n
    |n > 26 = cifrado frase (tail lista) 0
    |frase == (head lista) = cifrar (head lista) n
    |otherwise = cifrado frase lista (n+1) 

descifrable :: String -> [String] -> Bool
descifrable _ [] = False
descifrable frase lista
    |esDescifrado frase (head lista) = True
    |otherwise = descifrable frase (tail lista)
